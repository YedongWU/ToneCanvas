...
def semitones_to_frequency(semitones, reference_frequency=85):
    ...
    frequencies = np.zeros_like(semitones)
    frequencies[valid] = reference_frequency * (2 ** (semitones[valid] / 12))
    frequencies[~valid] = 0  # 将无效频率值设置为 0
    return frequencies
...
# 将半音转换为频率
all_frequencies = semitones_to_frequency(all_pitches_semitones)

# 生成纯音音频
duration = all_times[-1] - all_times[0]
tone_audio = generate_tone(all_frequencies, duration, sample_rate)

# 保存生成的纯音音频
sf.write('generated_tone.wav', tone_audio, sample_rate)
